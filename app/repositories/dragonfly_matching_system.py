from app.utils.constants import DragonflyNamespace, DragonflyMatchingField, DragonflyMatchingStatus
from app import r
from app.dto.dragonfly_player import DragonflyPlayer
from typing import Union
from app.utils.dragonfly_helpers import decode_bytes, create_player_key, get_player_id_from_key
...
class DragonflyMatchSystemRepository:
    ...
    def add_user_to_waitroom(player: DragonflyPlayer) -> Union[None, str]:
        ...
        new_status = decode_bytes(r.hget(player_key, DragonflyMatchingField.STATUS.value))
        new_wait_time = decode_bytes(r.hget(player_key, DragonflyMatchingField.WAIT_TIME.value))

        return f"status: {new_status}, wait_time: {new_wait_time}"

    @staticmethod
    def get_two_oldest_players_ids():
        """
        Finds two oldest players in the waiting room.
        """
        # Search for all players w waiting status
        waiting_players_query_str = f'@status:{{waiting}}'
        ...
        oldest_player_ids = [get_player_id_from_key(doc.id) for doc in oldest_players.docs if doc.id is not None]

        return oldest_player_ids

    @staticmethod
    def change_matched_players_status_to_pending(player1_id:str, player2_id:str):
