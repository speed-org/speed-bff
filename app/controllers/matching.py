...
def add_player_to_waitroom(player_id):
    ...
    """
    player = DragonflyPlayer(player_id, DragonflyMatchingStatus.WAITING.value, int(time.time()))
...
def match_oldest_players():
    ...
    # Changing status
    players_status = DragonflyMatchSystemRepository.change_matched_players_status_to_pending(player_ids[0], player_ids[1])
